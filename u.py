...
class aSimpleExploratoryAttacker:
    ...
    def get_worst_fit_individual(self):
        ...
        worst_individual = -1
        for i in range(self.population_size):
            if self.population[i].fitness < worst_fitness:#这里population之所以有fitness属性是因为population里面的元素是individual
                                                          #而individual里面有这个属性
    ...
    def evolutionary_cycle(self):
        ...
        for j in range(self.chromosome_length):
            self.population[kid].chromosome[j] = random.uniform(self.population[mom].chromosome[j],
                                                                self.population[dad].chromosome[j])
